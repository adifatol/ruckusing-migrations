...
class Ruckusing_PostgresTableDefinition {
	...
	function __construct($adapter, $name, $options = array()) {
		//sanity check
		if( !($adapter instanceof Ruckusing_PostgresAdapter)) {
			throw new Ruckusing_MissingAdapterException("Invalid PostgresSQL Adapter instance.");
		}
		if(!$name) {
